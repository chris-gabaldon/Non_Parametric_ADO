...
list_probs_ni=np.mean(list_probs_per_subject_bay_ni,axis=0)
list_probs_g=np.mean(list_probs_per_subject_bay_g,axis=0)
...
for suj in range(num_subj):
    ...
    p = prior.set_prior_cuadra(d[-1],D,k) # generate a gaussian prior
...
list_y_data = [list_probs,list_probs_ni,list_probs_g, np.mean(list_estimated_ado_fre, axis=0), np.mean(list_estimated_ran_fre, axis=0),np.mean(list_estimated_ado_fre_gauss, axis=0),np.mean(list_estimated_ado_bay, axis=0), np.mean(list_estimated_ran_bay, axis=0),np.mean(list_estimated_ado_bay_gauss, axis=0)]
labels = ['Ground truth fre','Ground truth bay ni','Ground truth bay gauss', 'ADO-Non Informative -fre', 'Random-fre','ADO-Gaussian-fre','ADO-Non Informative -bay', 'Random-bay','ADO-Gaussian-bay']
colors = ['black', 'brown','yellow','b', 'r','g','c','m','lightgreen']
k
